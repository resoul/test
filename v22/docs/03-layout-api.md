...
- Нода, упомянутая дважды в одном проходе, — ошибка раскладки. Предложено: проход
  отклоняется целиком (как валидация C21 в Trellis), диагностика возвращается в результате.
  Сейчас движок только сообщает дубликаты в `LayoutResult.duplicateIDs` (кадр получает
  первое вхождение), слой нод их не проверяет — не сделано.
