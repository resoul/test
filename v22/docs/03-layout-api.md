...
Не сделано: отскок на Mac; VoiceOver на Mac — прокрутка к элементу и постоянные
элементы (`NodeNSView` пересобирает их после каждой отрисовки); полоса прокрутки в RTL — справа, как в LTR; постраничная
