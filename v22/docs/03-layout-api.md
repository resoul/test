...
Не сделано: фокус в Mac Catalyst не проверялся — пакет для Catalyst не собирается (#153:
адаптеры AppKit там компилируются, а `NSView` недоступен); прокрутка к ноде вне экрана —
прокрутки ещё нет.
