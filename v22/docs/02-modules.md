...
квалификацию типов (`Layout.X`) у всех, кто импортирует оба. Рабочее имя модуля раскладки —
`V22Layout`; окончательные имена — после решения «Trellis v2 или новая библиотека».
...
```
Layout          чистая математика и описание раскладки. Только Foundation. Не знает про ноды.
