...
- **Mac.** `NodeNSView.scrollWheel`: дельты колеса и трекпада (инерция трекпада приходит
  ...
  строками по 10 pt. Что прокруткам не нужно — дальше по цепочке ответчиков. Отскока нет.
- **tvOS.** Физики касаний нет — прокручивает фокус. Не сделано:
  `UIFocusItemScrollableContainer`, без которого движок фокуса не находит ноды за краем
  окна.
