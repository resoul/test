...
| `Sources/TrellisCore/Pager/TabbedScrollNode.swift`; `ContainerHost.applyScrollConfiguration` в `HostedContainer.swift` и `NodeHostBridge+Containers.swift`; передача колеса в `NSScrollViewBacking.swift`; `Tests/TrellisRenderTests/TabbedScrollNodeHostTests.swift`, `AppKitTabbedScrollTests.swift`; `Playground/Shared/Scenarios/S39_TabbedProfile.swift` | Telegram-iOS upstream `6ad963e` (`PeerInfoScreen.swift`, `PeerInfoPaneContainerNode.swift`, `SparseItemGrid.swift`) — ориентир модели, код не переносится ([telegram-peerinfo-analysis.md](telegram-peerinfo-analysis.md)) | заново: внешний scroll с блоком в высоту viewport, блокировка страниц до закрепления, правила позиций по ADR 0037; без приватного API и без передачи инерции (этап 2) | R14 |
