...
Статус на 2026-09-23: **этап 1 написан, не собран и не проверен.** Код написан в облачной
сессии на Linux без Swift и Xcode (`download.swift.org` закрыт сетевой политикой окружения).
Ни одна строка ниже не подтверждена компилятором или тестом; проверка — на Mac пользователя
по списку в разделе «Проверки». Карточка не закрыта; этап 2 (передача инерции) не начат.
...
Ничего из этого не запускалось.
...
- Этап 1 не собран и не проверен ни одним тестом (см. выше).
