...
- **iOS, iPadOS, Catalyst.** На каждую видимую прокрутку — пустой `UIScrollView` над её
  ...
  кодом, `UIScrollView` догоняет после отрисовки; под пальцем и в инерции — сдвигается на
  столько же, сколько код сдвинул прокрутку с их последнего согласия, и пан или инерция
  продолжаются оттуда (так якорь ленивого стека не теряется при перетаскивании). Сдвиг,
  сделанный до следующего события пальца, учитывается в нём. Покадровое движение
  (`NodeHost.needsFrames`, переход ленивого стека дальше экрана) ведёт `CADisplayLink` в
  режиме `.common`: `advanceFrames(to: targetTimestamp)`, затем обычная отрисовка; ссылка
  снимается, когда движений не осталось, и держит view слабо (`FrameTarget`). Палец двигает
  `UIScrollView` от текущего смещения и останавливает движение. `scrollsToTop` оставлен системным
...
- **Mac.** `NodeNSView.scrollWheel`: дельты колеса и трекпада (инерция трекпада приходит
  ...
  инерция у края отскакивает; колесо мыши упирается. Покадровое движение — от
  `NSView.displayLink(target:selector:)` (macOS 14), кадры приходят, пока view в окне на
  экране; колесо останавливает движение.
...
- **Секции фокуса внутри прокрутки** (2026-09-26, дефект #168). `UIFocusGuide` живёт в view и
  стоит там, где секция на экране; секцию за краем окна движок фокуса не находит. Поэтому
  секция внутри прокрутки — `SectionEntry`: фокусируемый элемент во всю её рамку среди
  элементов `ScrollFocusContainer`. Движок находит его по всему содержимому, прокручивает к
  нему и фокусирует; `NodeView` на следующем такте переводит фокус на ноду секции (последнюю
  фокусированную там, иначе первую) — запросом к себе (`preferredFocusEnvironments` отдаёт
  ноду). Запрос к элементу самой ноды UIKit молча игнорирует: элемент не содержит
  сфокусированный вход (контракт `requestFocusUpdate(to:)`). Пока фокус внутри секции, вход
  выключен и прозрачен для перекрытия. Секции вне прокрутки — прежние `UIFocusGuide`.
