...
| `AccessibilityEntry`, `AccessibilityList`, `AccessibilityListItem`; `NodeHost.accessibilityEntries`, `accessibilityList`, `accessibilityFrame(ofItem:in:)`, `revealItem`; `AccessibleList` (`LazyStack`) | ленивый стек для VoiceOver — список со всеми строками | [03](03-layout-api.md) (десятый срез, VoiceOver) |
| `ListAccessibilityContainer`, `ListItemStandIn` (UIKit) | контейнер `.list` с элементом на каждую строку; заглушка раскладывает строку, когда на неё встаёт VoiceOver | [03](03-layout-api.md) (десятый срез, VoiceOver); как `UICollectionView`; UI-тест `Demo/iOSUITests` |
